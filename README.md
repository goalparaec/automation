...
You upload the same master workbook you use today. The app reads only the
5 sheets those 4 reports actually depend on (`360_Daily`, `360_Cum`,
`Converted`, `Prepaid_Bill`, `IRCA_Bill`) — it never touches the huge unused
sheets (`PRPD_BPS`, `Non_Comm_Dash`, `-ve_Balance`, etc.), so the 89MB file
uploads and parses quickly. Supabase then does the same math the Excel
formulas did, and keeps full daily history.
...
2. Every day: open **Upload**, pick the date, choose that day's workbook,
   submit. The app parses it and generates all 4 outputs automatically.
3. Open **the report page** for that date to view/download:
...
4. History is kept — you can revisit any past date's report at any time.
