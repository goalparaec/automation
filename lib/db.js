...
const ONCONFLICT_BY_TABLE = {
  input_360_daily: 'report_date,sl_no,sub_division',
  input_360_cum: 'report_date,sl_no,sub_division',
  input_converted: 'report_date,sl_no,sub_division_name',
  input_prepaid_bill: 'report_date,sl_no,sub_division_name',
  input_irca_bill: 'report_date,sl_no',
};
...
/** Upserts parsed rows for one input table, tagging every row with report_date. */
export async function upsertInputRows(table, reportDate, rows) {
  const onConflict = ONCONFLICT_BY_TABLE[table];
  if (!onConflict) throw new Error(`No conflict key configured for table "${table}"`);
...
function sortBySubDivision(rows) {
  const order = [...SUB_DIVISIONS, 'Circle Total'];
