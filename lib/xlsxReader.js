 ...
 * upload) in .xlsx, .xls, or .csv format - SheetJS auto-detects the actual
 * format from the file's content, not its name/extension. The file may
 * contain just one sheet - possibly named "Sheet1" or anything else by
 * whatever system generated it - so this doesn't require an exact
 * sheet-name match: it tries to find a sheet whose name loosely matches
 * sheetType, and otherwise just reads the first sheet in the file.
 * @param {Buffer} buffer
...
export async function readSingleSheetFile(buffer, sheetType) {
  ...
  try {
    ...
    throw new Error(`Could not read this file as .xlsx, .xls, or .csv: ${err.message}`);
  }
  ...
  // Array-of-arrays: rawRows[rowIndex][colIndex], colIndex 0 = column A.
  const rawRows = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: true, defval: null, blankrows: false });
  ...
  for (const row of rawRows) {
    ...
    const stopValue = typeof rawStop === 'number' ? rawStop : parseFloat(rawStop);
    ...
    for (const [letter, field] of Object.entries(config.columns)) {
      ...
      if (config.dateFields.includes(field)) {
        ...
        record[field] = typeof value === 'number' ? value : parseFloat(value);
      }
    }
